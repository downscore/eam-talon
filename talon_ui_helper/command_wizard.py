...
def handle_blob_detect_builder(result):
  ...
  if active_rectangle is None:
    raise Exception("Invalid active rectangle")
...
def open_overlay(index):
  ...
  _, overlay, handler, help_text = command_wizards[index]
  existing_overlay = overlay(handler, text=help_text)


@imgui.open(y=0)
def builder_picker(gui: imgui.GUI):
  gui.text("Choose the command type that you would like to build:")
  gui.spacer()
