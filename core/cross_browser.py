...
def _get_active_browser_interface() -> BrowserInterface:
  ...
  if chrome_app:
    return BrowserInterface(chrome_app, actions.user.chrome_get_all_tabs, actions.user.chrome_focus_tab,
                            actions.user.chrome_focus_tab_and_window, actions.user.chrome_is_focused,
                            actions.user.chrome_get_current_address)

  safari_app = actions.user.safari_get_app()
  if safari_app:
    return BrowserInterface(safari_app, actions.user.safari_get_all_tabs, actions.user.safari_focus_tab,
                            actions.user.safari_focus_tab_and_window, actions.user.safari_is_focused,
                            actions.user.safari_get_current_address)

  raise ValueError("Found no running browser.")


def _focus_tab_and_window_with_context(interface: BrowserInterface, context: browser_util.BrowserContext,
  ...
  window_id = context.window_ids[window_index - 1] if len(context.window_ids) >= window_index else None
  window = None
  if window_id:
  ...
  # Fall back to focus method that does not use a context if the window was not found. This method can be unreliable if
  # multiple windows or Chrome apps are open.
  if not window:
    print(f"Could not find window for changing browser tab. Window Index: {window_index}, Tab Index: {tab_index}")
...
class Actions:
  ...
  def cross_browser_focus_tab_and_window_with_context(context: browser_util.BrowserContext, window_index: int,
                                                      tab_index: int):
  ...
  def cross_browser_focus_tab_by_hostname(hostname: str):
    ...
    if not is_browser_focused and focused_tab_index is not None:
      ...
      _focus_tab_and_window_with_context(interface, context, matching_tabs[focused_tab_index].window_index,
