...
def get_image_template_directory():
  ...
  if maybe_value:
    ...
    return os.path.join(os.path.dirname(os.path.realpath(__file__)), "../image_templates")
...
def calculate_relative(modifier: str, start: float, end: float) -> float:
  """Helper method for settings. Lets you specify numbers relative to a range. For example:
...
@mod.action_class
class MouseActions:
  """Actions related to moving and clicking the mouse."""
  ...
  def mouse_helper_calculate_relative_rect(relative_rect_offsets: str,
    ...
    if region == "active_screen":
      ...
      raise ValueError("Unhandled region")
    if not base_rect:
      raise ValueError("Invalid base rect")

    mods = relative_rect_offsets.split(" ")
    calc_pos = calculate_relative
    x = calc_pos(mods[0], base_rect.x, base_rect.x + base_rect.width)
    y = calc_pos(mods[1], base_rect.y, base_rect.y + base_rect.height)
    rect = TalonRect(
        x,
        y,
        calc_pos(mods[2], base_rect.x, base_rect.x + base_rect.width) - x,
        calc_pos(mods[3], base_rect.y, base_rect.y + base_rect.height) - y,
    )
  ...
  def mouse_helper_find_template_relative(template_path: str,
                                          xoffset: float = 0,
                                          yoffset: float = 0,
                                          region: Optional[TalonRect] = None) -> List[TalonRect]:
    ...
    if os.pathsep in str(template_path):  # Cast to str to prevent linter error.
    ...
    matches = [
        TalonRect(match.x + xoffset, match.y + yoffset, match.width, match.height)
        for match in locate.locate(template_file, rect=rect, threshold=0.999)
    ]

    return sorted(matches, key=lambda m: (m.x, m.y))
  ...
  def mouse_helper_blob_picker(bounding_rectangle: TalonRect, min_gap_size: int = 5):
    ...
    current_image = screencap_to_image(bounding_rectangle)
    rects = calculate_blob_rects(current_image, bounding_rectangle, min_gap_size=min_gap_size)
