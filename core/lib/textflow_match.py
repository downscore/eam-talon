...
def get_nth_regex_match(text: str, regex: re.Pattern, n: int,
                        direction: SearchDirection) -> Optional[re.Match]:
  """Get the nth (1-based) match of a regex in the given text. Returns None if there is no nth
  match."""
  if n < 1:
...
def get_nth_substring_match(text: str, search: str, n: int,
                            direction: SearchDirection) -> Optional[re.Match]:
  """Get the nth (1-based) match of a substring in the given text. Returns None if there is no nth
  match."""
  return get_nth_regex_match(text, re.compile(re.escape(search), re.IGNORECASE), n, direction)


def get_nth_word_start_match(text: str, search: str, n: int,
                             direction: SearchDirection) -> Optional[TextMatch]:
  """Matches the given substring to the start of a word in the given text. Returns None if no match
  was found."""
  # No match for empty string.
  if len(search) == 0:
    return None

  # If the search string starts with a symbol (besides underscore), don't look for the start of a
  # word. Underscore is exempt because it is often used to start symbol names in code.
  ...
  # Create a regex from the substring that only matches that start of words. Match is case
  # insensitive.
...
def get_nth_line_start_match(text: str, search: str, n: int,
                             direction: SearchDirection) -> Optional[TextMatch]:
  """Matches the given substring to the start of a line in the given text. Returns None if no match
  was found."""
  # No match for empty string.
  if len(search) == 0:
    return None
...
def expand_match_to_token(text: str, match: TextMatch) -> TextMatch:
  """Expands the given match to cover a whole token. Extends to either side until a
  non-alphanumeric, non-separator char or string end is encountered. Note: Does not treat dots or
  dashes as a separator. Each "word" in "word.word-word" is a token."""
...
def _match_token_partial(text: str, options: TokenMatchOptions, direction: SearchDirection,
  ...
  """Finds a partial (may be full) match for a token in the given text using the given options.
  Returns the range or None if a matching token could not be found."""
  ...
  if options.match_method == TokenMatchMethod.PHRASE:
    ...
    phrase_match = get_nth_regex_match(text, re.compile(regex, re.IGNORECASE), options.nth_match,
                                       direction)
  ...
  if options.match_method in (TokenMatchMethod.LINE_START,
  ...
  if options.match_method == TokenMatchMethod.EXACT_WORD:
    ...
    word_match = get_nth_regex_match(text, re.compile(regex, re.IGNORECASE), options.nth_match,
                                     direction)
  ...
  if options.match_method in (TokenMatchMethod.WORD_START,
                              TokenMatchMethod.WORD_START_THEN_SUBSTRING,
                              TokenMatchMethod.LINE_START_THEN_WORD_START_THEN_SUBSTRING):
...
def match_token(text: str, options: TokenMatchOptions, direction: SearchDirection,
                get_homophones: Callable[[str], list[str]]) -> Optional[TextMatch]:
  """Matches a token in the given text using the given options. Returns the match range or None if a
  matching token could not be found.
  `get_homophones`: Used for homophone expansion when matching by phrase. Given a word, the function
  should return a list containing the word and its homophones."""
