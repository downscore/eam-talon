...
class GetPhraseRegexTestCase(unittest.TestCase):
  ...
  def test_get_phrase_regex(self):
    ...
    self.assertEqual(get_phrase_regex("a b".split(" "), _get_homophones_mock),
...
class MatchNthWordStartSubstringTestCase(unittest.TestCase):
  ...
  def test_first_word_forward(self):
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "p", 1, SearchDirection.FORWARD),
                     make_text_match(0, 1))
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "pat", 1, SearchDirection.FORWARD),
                     make_text_match(0, 3))

  def test_second_match_forward(self):
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "p", 2, SearchDirection.FORWARD),
                     make_text_match(25, 26))
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "pat", 2, SearchDirection.FORWARD),
                     make_text_match(95, 98))

  def test_second_word_forward(self):
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "f", 1, SearchDirection.FORWARD),
                     make_text_match(8, 9))
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "find", 1, SearchDirection.FORWARD),
                     make_text_match(8, 12))
  ...
  def test_no_match_not_word_start(self):
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "attern", 1, SearchDirection.FORWARD),
                     None)
    self.assertEqual(get_nth_word_start_match(_TEXT_BEFORE, "earch", 1, SearchDirection.BACKWARD),
                     None)
    self.assertEqual(get_nth_word_start_match(_TEXT_AFTER, "attern", 2, SearchDirection.FORWARD),
                     None)
    self.assertEqual(get_nth_word_start_match(_TEXT_BEFORE, "earch", 2, SearchDirection.BACKWARD),
                     None)
  ...
  def test_non_starting_then_starting_substring(self):
    ...
    self.assertEqual(get_nth_word_start_match(_TEXT_BEFORE, "re", 1, SearchDirection.BACKWARD),
  ...
  def test_fragments_are_not_words(self):
    ...
    self.assertEqual(get_nth_word_start_match(text, "ty", 1, SearchDirection.BACKWARD),
                     make_text_match(6, 8))

  def test_symbol_search_ignores_word_starts(self):
    # First ] should be matched, not the second one.
    text = "combo]]"
    self.assertEqual(get_nth_word_start_match(text, "]", 1, SearchDirection.FORWARD),
                     make_text_match(5, 6))
  ...
  def test_underscore_search_respects_word_starts(self):
    ...
    self.assertEqual(get_nth_word_start_match(text, "_s", 1, SearchDirection.FORWARD),
                     make_text_match(13, 15))
...
class MatchNthLineStartSubstringTestCase(unittest.TestCase):
  ...
  def test_first_line_forward(self):
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "p", 1, SearchDirection.FORWARD),
                     make_text_match(0, 1))
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "pat", 1, SearchDirection.FORWARD),
                     make_text_match(0, 3))

  def test_second_line_forward(self):
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "s", 1, SearchDirection.FORWARD),
                     make_text_match(41, 42))
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "sim", 1, SearchDirection.FORWARD),
                     make_text_match(41, 44))
  ...
  def test_second_line_backward(self):
    self.assertEqual(get_nth_line_start_match(_TEXT_BEFORE, "f", 1, SearchDirection.BACKWARD),
                     make_text_match(20, 21))
  ...
  def test_no_match_not_line_start(self):
    ...
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "attern", 1, SearchDirection.FORWARD),
                     None)
    self.assertEqual(get_nth_line_start_match(_TEXT_BEFORE, "h", 1, SearchDirection.BACKWARD), None)
    self.assertEqual(get_nth_line_start_match(_TEXT_BEFORE, "his", 1, SearchDirection.BACKWARD),
                     None)
    self.assertEqual(get_nth_line_start_match(_TEXT_AFTER, "attern", 2, SearchDirection.FORWARD),
                     None)
    self.assertEqual(get_nth_line_start_match(_TEXT_BEFORE, "his", 2, SearchDirection.BACKWARD),
                     None)

  def test_indented_lines(self):
    self.assertEqual(get_nth_line_start_match(_SIMILAR_LINES, "in", 1, SearchDirection.FORWARD),
                     make_text_match(2, 4))
...
class ExpandMatchToTokenTestCase(unittest.TestCase):
  ...
  def test_first_token(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(0, 0)),
                     make_text_match(0, 12))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(0, 1)),
                     make_text_match(0, 12))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(0, 12)),
                     make_text_match(0, 12))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(1, 11)),
                     make_text_match(0, 12))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(9, 10)),
                     make_text_match(0, 12))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(12, 12)),
                     make_text_match(0, 12))

  def test_snake_case(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(13, 13)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(13, 14)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(14, 14)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(14, 26)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(14, 27)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(27, 27)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(19, 22)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(18, 21)),
                     make_text_match(13, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(18, 19)),
                     make_text_match(13, 27))

  def test_camel_case(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(28, 28)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(28, 29)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(29, 29)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(29, 39)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(29, 40)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(40, 40)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(33, 37)),
                     make_text_match(28, 40))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(32, 36)),
                     make_text_match(28, 40))

  def test_kebab_case(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(41, 41)),
                     make_text_match(41, 46))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(41, 42)),
                     make_text_match(41, 46))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(42, 42)),
                     make_text_match(41, 46))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(42, 53)),
                     make_text_match(41, 54))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(42, 54)),
                     make_text_match(41, 54))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(41, 54)),
                     make_text_match(41, 54))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(47, 50)),
                     make_text_match(47, 50))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(46, 49)),
                     make_text_match(46, 50))

  def test_dot_separated(self):
    # Note: Dots are not treated as separator characters. Each dot-separated component is its own
    # token.
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(55, 58)),
                     make_text_match(55, 58))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(55, 55)),
                     make_text_match(55, 58))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(55, 56)),
                     make_text_match(55, 58))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(56, 56)),
                     make_text_match(55, 58))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(58, 58)),
                     make_text_match(55, 58))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(59, 62)),
                     make_text_match(59, 62))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(59, 59)),
                     make_text_match(59, 62))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(62, 62)),
                     make_text_match(59, 62))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(63, 67)),
                     make_text_match(63, 67))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(63, 63)),
                     make_text_match(63, 67))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(63, 64)),
                     make_text_match(63, 67))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(67, 67)),
                     make_text_match(63, 67))

  def test_pascal_case(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(72, 91)),
                     make_text_match(72, 91))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(72, 72)),
                     make_text_match(72, 91))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(72, 73)),
                     make_text_match(72, 91))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(91, 91)),
                     make_text_match(72, 91))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(78, 81)),
                     make_text_match(72, 91))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(81, 83)),
                     make_text_match(72, 91))

  def test_spanning_multiple_tokens(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(1, 20)),
                     make_text_match(0, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(0, 27)),
                     make_text_match(0, 27))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(1, 92)),
                     make_text_match(0, 92))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(0, 92)),
                     make_text_match(0, 92))

  def test_non_separator_symbol(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(68, 68)),
                     make_text_match(68, 68))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(68, 69)),
                     make_text_match(68, 69))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(67, 69)),
                     make_text_match(67, 69))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(67, 70)),
                     make_text_match(67, 70))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(68, 70)),
                     make_text_match(68, 70))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(69, 70)),
                     make_text_match(69, 70))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(91, 92)),
                     make_text_match(91, 92))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(92, 92)),
                     make_text_match(92, 92))
    # Matching a non-separator symbol (space, dot) between tokens. Should not expand to adjacent
    # tokens.
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(12, 13)),
                     make_text_match(12, 13))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(58, 59)),
                     make_text_match(58, 59))

  def test_separator_symbol(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(70, 70)),
                     make_text_match(70, 71))
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(70, 71)),
                     make_text_match(70, 71))

  def test_apostrophe(self):
    self.assertEqual(expand_match_to_token(_FORMATTED_TOKENS, make_text_match(93, 95)),
                     make_text_match(93, 98))
...
class MatchTokenTestCase(unittest.TestCase):
  ...
  def _token(self, text: str, options: TokenMatchOptions,
             direction: SearchDirection) -> Optional[str]:
  ...
  def test_empty_string(self):
    ...
    self.assertEqual(
        match_token(_TEXT_BEFORE, TokenMatchOptions(), SearchDirection.BACKWARD,
                    _get_homophones_mock), make_text_match(len(_TEXT_BEFORE), len(_TEXT_BEFORE)))
    self.assertEqual(
        match_token(_TEXT_AFTER, TokenMatchOptions(), SearchDirection.FORWARD,
                    _get_homophones_mock), make_text_match(0, 7))

  def test_first_token_forward(self):
    options = TokenMatchOptions(match_method=TokenMatchMethod.TOKEN_COUNT, nth_match=1)
    self.assertEqual(
        match_token(_TEXT_AFTER, options, SearchDirection.FORWARD, _get_homophones_mock),
        make_text_match(0, 7))
  ...
  def test_word_start_then_substring_no_match(self):
    options = TokenMatchOptions(match_method=TokenMatchMethod.WORD_START_THEN_SUBSTRING,
                                search="xyz")
    self.assertEqual(self._token(_TEXT_AFTER, options, SearchDirection.FORWARD), None)
    self.assertEqual(self._token(_TEXT_BEFORE, options, SearchDirection.BACKWARD), None)
  ...
  def test_word_substring(self):
    ...
    options = TokenMatchOptions(match_method=TokenMatchMethod.WORD_SUBSTRING,
                                search="do",
                                nth_match=1)
  ...
  def test_line_start_word_start(self):
    ...
    options = TokenMatchOptions(
        match_method=TokenMatchMethod.LINE_START_THEN_WORD_START_THEN_SUBSTRING,
        search="do",
        nth_match=1)
    self.assertEqual(self._token(text, options, SearchDirection.FORWARD), "done")
  ...
  def test_phrase(self):
    options = TokenMatchOptions(match_method=TokenMatchMethod.PHRASE,
                                search="var name",
                                nth_match=1)
    self.assertEqual(self._token(_FORMATTED_TOKENS, options, SearchDirection.FORWARD),
                     "snake_var_name")
  ...
  def test_word_start_does_not_match_inside_camel(self):
    # We may want to change this behavior: Word start does not currently match inside of
    # camel/pascal case tokens.
  ...
  def test_substring_interspersed_with_word_start(self):
    # Note: This may be confusing behavior. Word start match is performed first. If words with
    # substring matches are interspersed, they are skipped over in the first match attempt.
