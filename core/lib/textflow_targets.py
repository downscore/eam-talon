...
def _add_to_match(match: TextMatch, num_chars: int) -> TextMatch:
  """Returns a new text match with the given number of characters added to the given match's
  ranges."""
  result = TextMatch(TextRange(match.text_range.start + num_chars,
                               match.text_range.end + num_chars))
  if match.deletion_range is not None:
    result.deletion_range = TextRange(match.deletion_range.start + num_chars,
                                      match.deletion_range.end + num_chars)
  return result
...
def _combine_ranges(range_from: TextRange, unmapped_range_to: TextRange,
  ...
  unmapped_end = (unmapped_range_to.start
                  if combo_type == TargetCombinationType.UNTIL_TO else unmapped_range_to.end)
...
def _combine_matches(match_from: TextMatch, unmapped_match_to: TextMatch,
  ...
  result = TextMatch(
      _combine_ranges(match_from.text_range, unmapped_match_to.text_range, combo_type))
  # TODO: Handle deletion ranges if necessary.
  return result


def _get_best_token_match(
    target: SimpleTarget, text_before: str, text_after,
    utility_functions: UtilityFunctions) -> Optional[Tuple[SearchDirection, TextMatch]]:
  """Finds the best matching token. Does not implement priority of word starts over substrings
  across directions."""
  match_before = None
...
def _match_simple_target(
    target: SimpleTarget, text_before: str, text_after,
    utility_functions: UtilityFunctions) -> Optional[Tuple[SearchDirection, TextMatch]]:
  """Match a simple target in the given text. Returns None if no match was found. Returned direction
  is `BACKWARD` if the match is in `text_before` and `FORWARD` if the match is in `text_after`."""
...
def match_compound_target(target: CompoundTarget, text: str, selection_range: TextRange,
  ...
  """Match a compound target in the given text. Does not apply modifiers. Returns None if no match
  was found."""
  ...
  # Match 'from' simple target. If no 'from' target supplied, use the beginning of the current
  # selection.
  match_tuple_from: Optional[Tuple[SearchDirection, TextMatch]] = (SearchDirection.BACKWARD,
                                                                   TextMatch(selection_range))
  if target.target_from is not None:
    match_tuple_from = _match_simple_target(target.target_from, text_before, text_after,
                                            utility_functions)
  if match_tuple_from is None:
    return None

  # Map the 'from' match to the original text input.
  match_from = match_tuple_from[
      1]  # Note: No need to modify the range if it was matched in `text_before`.
  ...
  # Note: We allow searching through the selected text for the 'to' target. For this search, there
  # is no 'text_before' since we need to match after the 'from' target.
  match_tuple_to = _match_simple_target(target.target_to, "", text[match_from.text_range.end:],
                                        utility_functions)
